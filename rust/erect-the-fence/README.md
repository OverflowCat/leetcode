...
impl Solution {
    ...
    pub fn outer_trees(trees: Vec<Vec<i32>>) -> Vec<Vec<i32>> {
        ...
        let mut trees: Vec<Tree> = trees.into_iter()
            .map(|t| Tree(t[0] - init.0, t[1] - init.1)).collect();
        trees.sort_by(
        ...
        stack.push(trees[0]); stack.push(trees[1]);
        ...
        for x in stack.windows(2) {
            ...
            trees = trees.into_iter().filter_map(|a| {
                ...
                { extra.push(a.clone()); None } else { Some(a) }
            }).collect();
        }
        stack.pop(); stack.into_iter().chain(extra.into_iter())
...
Output
[[3,0],[4,0],[5,0],[6,1],[7,2],[7,3],[7,4],[6,5],[5,5],[4,5],[3,5],[2,5],[1,4],[0,3]]
Expected
